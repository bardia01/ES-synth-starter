...
SemaphoreHandle_t ghandshakeMutex;
...
void CANDecodeTask(void * pvParameters){
  ...
  while(1){
    ...
    xSemaphoreTake(ghandshakeMutex, portMAX_DELAY);
    bool cond1 = (g_HSWest == 1) && (g_HSEast == 0);
    bool cond2 = (g_HSEast ==0) && (g_HSWest == 0);
    bool cond3 = (g_HSEast == 1) && (g_HSWest ==0);
    uint8_t prev_isThree = g_isThree;
    uint8_t prev_isTwo = g_isTwo;
    uint8_t prev_myPos = g_myPos;
    xSemaphoreGive(ghandshakeMutex);

    if((localRX_Message[0] == 8) && MY_ID == 0){ // this message was from the receiver
      int posDif = prev_myPos - localRX_Message[4];
      l_myOct = localRX_Message[1] + posDif;
      l_myVol = localRX_Message[5];
    ...
    if(cond1){ 
      ...
      //memcpy(tmp_outBits, l_outBits, sizeof l_outBits);
        ...
        l_myPos = max((int)prev_myPos, 2);
        if(localRX_Message[6] == 1) l_myPos = 3;
        if(localRX_Message[7] == 1) l_myPos = 2;
        if(prev_isThree == 1){
          l_myPos = 2;
          l_isThree = 0;
          __atomic_store(&g_isThree, &l_isThree, __ATOMIC_RELAXED);
...
void loop() {
  ...
  //Serial.println(g_myPos, DEC);
  Serial.println(RX_Message[7], DEC);
  // if (millis() > next) {
